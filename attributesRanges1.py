...
class attributesRanges1:
    ...
    def generatePossibleValues_equalDepth(self, df_original, UserpredicateList, sorted_possible_refinments1):
        """
        ...
        for predicate in UserpredicateList:
            ...
            if data_type == 'numerical':
                if ccc == 1:
                    num_values = 23  # Desired number of bins
                if ccc == 2:
                    num_values = 6
                if ccc == 3:
                    num_values = 1
                ccc += 1
                pred = sorted(pred)  # Sort the data for equal-depth binning
                
                print(num_values, len(pred), "---------")

                bin_size = math.ceil(len(pred) / num_values)
                predPossibleValues = []

                # Create bins with equal numbers of data points
                for i in range(0, len(pred), bin_size):
                    bin_range = pred[i:i + bin_size]
                    if bin_range:
                        start, end = bin_range[0], bin_range[-1]
                        predPossibleValues.append({"range": (start, end)})

                # Calculate min_distance and max_distance for each bin
                concrete_values_in_range = []
                for rng in predPossibleValues:
                    # Find concrete values from sorted_possible_refinments1 within the range
                    concrete_values = []
                    for refinement in sorted_possible_refinments1:
                        for r in refinement['refinements']:
                            if r['column'] == column_name and rng['range'][0] <= r['value'] <= rng['range'][1]:
                                ...
                                new_entry = {"value": r['value'], "distance": distance_to_user_query}
                                if new_entry not in concrete_values:
                                    concrete_values.append(new_entry)

                    concrete_values_in_range.append(concrete_values)

                # Calculate min_distance for each range
                predPossibleValues = self.calculate_distances(predPossibleValues, concrete_values_in_range, pred_value)
                
            elif data_type == 'categorical':
                # Similar handling as before for categorical data
                predPossibleValues = [(val, val) for val in pred]
                user_pred_value = next((p[2] for p in UserpredicateList if p[0] == column_name), None)
                if user_pred_value is not None:
                    predPossibleValues = sorted(predPossibleValues, key=lambda x: self.calculate(x[0], user_pred_value))
                ...
                for val in predPossibleValues:
                    ...
                    concrete_values = [r['value'] for refinement in sorted_possible_refinments1 for r in refinement['refinements']
                                       if r['column'] == column_name and r['value'] == most_similar_value]
                    concrete_values = list(set(concrete_values))
                    concrete_values_in_range.append(concrete_values)
            else:
