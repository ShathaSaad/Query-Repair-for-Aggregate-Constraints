...
class filtered_partially:
    ...
    def check_predicates_partial_modified(self, sorted_possible_refinements, statistical_tree, expression, datasize, dataName):
        ...
        for refinement in sorted_possible_refinements:
            ...
            for root_key in root_clusters:
                filtered_clusters, counter, child_counter = self.filter_clusters_partial_modified(root_key, parent_child_map, cluster_map, conditions, operators, filtered_clusters, counter, child_counter)
            ...
            check_counter+=1
            satisfied, agg_counter = self.evaluate_constraint1.calculate_expression_partially(filtered_df, conditions, agg_counter, expression, "partial", similarity)
            ...
            if satisfied != []:
                if satisfied['Range Satisfaction'] == 'Full':
                    ...
                    check_counter+=1
                    for root_key in root_clusters:
                        filtered_clusters, counter = self.filter_fully.filter_clusters_Hash(root_key, cluster_map, conditions, operators, filtered_clusters, counter, parent_child_map)
                       
                    filtered_df = pd.DataFrame(filtered_clusters)
                    satisfied, agg_counter = self.evaluate_constraint.evaluate_constraint1(filtered_df, expression, conditions, agg_counter, similarity, "full")
            ...
            filename = f'filtered_partially_{conditions}.csv'
    ...
    def filter_clusters_partial_modified(self, cluster_key, parent_child_map, cluster_map, conditions, operators, filtered_clusters, counter, child_counter, coverage_threshold=75):
        stack = [cluster_key]
        lower_bound = 0
        ...
        while stack:
            ...
            for i, (condition, operator) in enumerate(zip(conditions, operators)):
                if not self.applyOperator.apply_operator(data_Min[i], data_Max[i], condition, operator, "Full"):
                    fully_satisfies = False
                if not self.applyOperator.apply_operator(data_Min[i], data_Max[i], condition, operator, "Partial"):
            ...
            if fully_satisfies:
                ...
                if cluster_key in parent_child_map:
                    ...
                    for child_key in parent_child_map[cluster_key]:
                        ...
                        for i, (condition, operator) in enumerate(zip(conditions, operators)):
                            if not self.applyOperator.apply_operator(child_Min[i], child_Max[i], condition, operator, "Full"):
                                child_fully_satisfies = False
                            if not self.applyOperator.apply_operator(child_Min[i], child_Max[i], condition, operator, "Partial"):
